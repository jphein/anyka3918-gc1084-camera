...
| ⚠️ Automatic day/night | **Works on cam #2; broken on cam #1** — cover cam #2's lens and the IR ring lights. ✅ **The old "not fixable on this board" verdict is RETRACTED**: it rested on `ain0` being a constant, and `ain0` is a **working light sensor** (2999 lit → ~130 covered, two events). It only *read* constant because nobody ever varied the light. `gpio-rf_feed` is still genuinely absent, so the *driver* path may still be unreachable — but the sensor input exists. [Full retraction](docs/ptz.md#-automatic-daynight-is-not-fixable-on-this-board) |

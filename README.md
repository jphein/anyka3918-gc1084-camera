...
| ⚠️ IR LEDs | **Unit-dependent, and the pin was never the mechanism.** Cam #2's ring **works** — covering the lens makes the firmware's own auto day/night switch light it. Cam #1's stays dark under the identical test. `IR_LED` is a **decoy on both**: the pad swings, nothing happens, and the firmware illuminates *without touching it or the IR-cut pins*. Day/night config is byte-identical across the two units, so the difference is **hardware, not software** — [full result](docs/ptz.md#-ir-confirmed-dark) |

...
| ✅ IR-cut filter | Works — manually. **Write `/sys/user-gpio/ircut_a` directly**; that is what `ctl` and the Home Assistant switch now do. [Every vendor route is dead](docs/ptz.md#-one-write-works-every-vendor-route-is-dead-and-they-die-in-the-same-place). ⚠️ **The boot-time write does NOT run** — it is appended to `Factory/config.sh` after `gergehack.sh`, which never returns, so expect magenta after a reboot until something sets the pin. [Measured](docs/sd-card.md#what-gets-fixed) |
| ⚠️ Automatic day/night | **Works on cam #2; broken on cam #1 — and the cause is measured.** ✅ **"Not fixable on this board" is RETRACTED.** It rested on `ain0` reading a constant; on cam #2 `ain0` is a **working light sensor** (2999 lit → ~130 covered, two events). On cam #1 it is genuinely dead — **2999 across all 77 samples**, one distinct value, through a full cover/uncover. `ain0` is the driver's fallback input, so a dead sensor means the guard never trips. `gpio-rf_feed` is still absent and the `cfg[0x1c]` gate is untested. **The old claim was an accurate measurement of cam #1 that silently became a claim about the board.** [Full retraction](docs/ptz.md#-automatic-daynight-is-not-fixable-on-this-board) |
| ⚠️ IR LEDs | **Unit-dependent, and the pin was never the mechanism.** Cam #2's ring **works**: covering the lens lights it, cam #1's stays dark under the identical test — both observed directly. `IR_LED` is a **decoy on both** — the pad swings and nothing happens. **What drives the ring is not established**: a 2 s sampler recorded no change on `IR_LED`, `ircut_a` or `ircut_b` across the whole transition, which argues *against* a software path rather than for one. Day/night config is byte-identical across the two units, so the divergence is **hardware** — [full result](docs/ptz.md#-ir-confirmed-dark) |
| ❌ White LEDs | Present in hardware (4 on the ring) but dark — now **confirmed on two separate units**, so this is variant-wide and not one bad board. The **pad demonstrably swings** and nothing lights, the vendor firmware **declares this PTZ variant unsupported**, and there is **no software fix** — [all other candidates refuted](docs/ptz.md#-white-leds--the-vendor-firmware-disables-them-on-this-variant) |
...
| ⚠️ Identity | The design — **name yourself from your own MAC**, write-once, no registry, name stays with the camera while the build follows the card — ships and is sound. **But the first-boot hook cannot execute**: it is appended to `Factory/config.sh` after `gergehack.sh`, which never returns. `/data/unit.json` exists on neither camera. [How it is meant to work](docs/identity.md) · [why it doesn't yet](docs/sd-card.md#what-gets-fixed) |
